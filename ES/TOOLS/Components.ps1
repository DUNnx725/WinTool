...
Write-Host 'COMPONENTES OPCIONALES'
Write-Host ''
foreach($i in $items){
 Write-Host ('{0,-24} {1,-12} {2}' -f $i.Name,$(if($i.Found){'DISPONIBLE'}else{'NO INSTALADO'}),$i.How)
}
Write-Host ''
Write-Host 'WinTool no descarga ejecutables desconocidos ni instala componentes sin confirmacion.'
exit 0
