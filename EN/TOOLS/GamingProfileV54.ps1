...
if($Mode -eq 'Analyze'){
    ...
    Write-Host 'GAMING PROFILE - ANALYSIS'
    Write-Host ''
    Write-Host ('Windows................ '+$os.Caption.Replace('Microsoft ','')+' Build '+$os.BuildNumber)
    Write-Host ('Power plan.............. '+(ActivePlanDisplay))
    Write-Host ('Transparency............ '+$(if($trans.Exists -and [int]$trans.Value -eq 0){'DISABLED'}else{'ENABLED / DEFAULT'}))
    Write-Host ('Visual effects.......... '+$(if($visual.Exists -and [int]$visual.Value -eq 2){'PERFORMANCE PRIORITY'}else{'NORMAL / CUSTOM'}))
    Write-Host ('Game DVR recording...... '+$(if(($dvr.Exists -and [int]$dvr.Value -eq 0) -or ($cap.Exists -and [int]$cap.Value -eq 0)){'DISABLED'}else{'ENABLED / DEFAULT'}))
    Write-Host ('Game Mode.............. '+$(if($gm.Exists -and [int]$gm.Value -eq 1){'ACTIVE'}else{'DEFAULT'}))
    Write-Host ''
    Write-Host 'The Maximum Performance profile can:'
    Write-Host '- disable transparency and window animations;'
    Write-Host '- favor performance-oriented visual effects;'
    Write-Host '- disable Game DVR background recording;'
    Write-Host '- enable Game Mode;'
    Write-Host '- use Windows Ultimate Performance;'
    Write-Host '- does not modify internal Windows power-plan values.'
    Write-Host ''
    Write-Host 'Does not change priorities, affinities, HPET, BCD, pagefile, MSI Mode or critical services.'
    exit 0
}
...
if($Mode -eq 'Apply'){
    ...
    if($ultimateGuid){
        ...
        Write-Host '[CHECK] Windows could not locate or create Ultimate Performance.'
        Write-Host '[INFO] The rest of the Gaming Profile was applied without changing the power plan.'
    }
    ...
    Write-Host '[OK] Gaming Profile applied.'
    Write-Host ''
    Write-Host 'Main changes:'
    Write-Host '- lighter interface;'
    Write-Host '- Game DVR background recording disabled;'
    Write-Host '- Game Mode enabled;'
    Write-Host '- Windows Ultimate Performance selected when available;'
    Write-Host '- the power plan keeps its official Windows values.'
    Write-Host ''
    Write-Host 'It may increase power use and temperatures. Some visual changes complete after signing in again.'
    exit 0
}

if($Mode -eq 'Restore'){
    if(-not (Test-Path $backupFile)){
        Write-Host '[INFO] No Gaming Profile backup was found.'
        exit 0
    }
    try{$backup=Get-Content -LiteralPath $backupFile -Raw|ConvertFrom-Json}catch{
        Write-Host '[ERROR] The backup could not be read.'
        exit 2
    }
    ...
    Write-Host '[OK] Gaming Profile restored to the saved state.'
    exit 0
}
