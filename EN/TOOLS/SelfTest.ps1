...
function CheckDir([string]$p,[string]$name){
    ...
    try{
        ...
        Add("[OK] $name available.")
    }catch{
        Add("[ERROR] $name is not available for writing.")
        $script:bad++
    }
}
...
try{
    ...
    try{
        ...
        if([Environment]::OSVersion.Version.Major -ge 10){
            Add('[OK] Compatible system: '+$caption)
        }else{
            Add('[ERROR] WinTool requires Windows 10 or 11.')
            $bad++
        }
    }catch{
        Add('[CHECK] Windows version could not be read through CIM.')
    }
    if([Environment]::Is64BitOperatingSystem){
        Add '[OK] Arquitectura x64.'
    }else{
        Add '[ERROR] WinTool requires 64-bit Windows.'
        $bad++
    }

    Add ''
    Add 'Checking working folders...'
    CheckDir $ReportsDir 'REPORTS'
    CheckDir $BackupsDir 'BACKUPS'
    CheckDir $ConfigDir 'CONFIG'
    CheckDir $logDir 'LOGS'

    Add ''
    Add 'Checking PowerShell scripts...'
    ...
    Add ''
    Add 'Checking required components...'
    $required=@(
    ...
    if($PresentMonPath){
        if(Test-Path -LiteralPath $PresentMonPath){
            ...
            try{
                ...
                if($h -eq $expected){
                    Add '[OK] PresentMon 2.5.1 verified by SHA-256.'
                }else{
                    Add '[ERROR] PresentMon does not match the verified component.'
                    $bad++
                }
            }catch{
                Add '[ERROR] PresentMon SHA-256 could not be verified.'
                $bad++
            }
        }else{
            Add '[ERROR] PresentMon not found.'
            $bad++
        }
    }
            ...
            foreach($line in $raw){
                if($line -match '^\s*:([A-Za-z0-9_]+)\s*$'){
                    ...
                    if($labels.ContainsKey($key)){
                        Add('[ERROR BAT] Duplicate label: '+$key)
                        $bad++
                    }else{
                        $labels[$key]=$true
                    }
            ...
            foreach($line in $raw){
                if($line -match '(?i)\bgoto\s+([A-Za-z0-9_]+)'){
                    ...
                    if(-not $labels.ContainsKey($g)){
                        Add('[ERROR BAT] GOTO without target: '+$g)
                        $bad++
                    }
                }
                if($line -match '(?i)^\s*if\s+errorlevel.+&\s*(goto|pause|start|powershell|cmd)'){
                    Add('[ERROR BAT] IF ERRORLEVEL with chained commands: '+$line.Trim())
                    $bad++
                }
            }
            if($bad -eq 0){ Add '[OK] Estructura BAT verificada.' }
        }else{
            Add '[ERROR BAT] Main BAT file was not found.'
            $bad++
        }
    }
    ...
    Add ''
    if($bad -eq 0){
        Add '[OK] Self-check completed with no blocking errors.'
        $rc=0
    }else{
        Add('[ERROR] Problems detected: '+$bad)
        $rc=2
    }
